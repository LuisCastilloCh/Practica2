...
int convertir_rom_arab(char letra){
    if(letra=='M'){
        return 1000;
    }
    if(letra=='D'){
...
int suma_NO_unidades(int indice,string num_romano){
    ...
    for(int au=0;true;au++){
        if(au==indice){
            suma+=convertir_rom_arab(num_romano[au]);
            return suma;
            break;
        }

        if(convertir_rom_arab(num_romano[au])<convertir_rom_arab(num_romano[au+1])){
            suma-=convertir_rom_arab(num_romano[au]);
        }
        else{
            suma+=convertir_rom_arab(num_romano[au]);
...
int main(){
    ...
    cout<<" Recuerda ingresar un numero entre 1-3999 "<<endl;
    cout<<" ingresa tu numero en romano: ";
    cin>>numero_romano;
